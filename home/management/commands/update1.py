    ...
    def handle(self, *args, **options):
        ...
        def sendshopping(user):
            ...
            for upline in uplines:
                ...
                if upline_user != 'blank' and directs >= level and sp.today_level_income <= 40000 and sp.amount >= 1500:
                    if sx.plan == 30:
                        upline_amount = levels30['level{}'.format(level)]
                    else:
                        upline_amount = levels70['level{}'.format(level)]
                    upline_user.shopping_wallet += upline_amount*0.9
        ...
        for idv in newids:
            ...
            if True: 
                ...
                if True:
                    ...
                    print('------------------------------------------------saveing-------------------------', userid)
                    # user.save()
                    # usewallet.save()
                try:
                    prime = BinaryTree.objects.get(user=user.username)
                except Exception as e:
                    print('Not Found')
                if plan == '30' and directs >= 2:
                    x = user.shopping_wallet
                    y = user.new_funds
                    v = user
                    v.new_funds += x
                    # v.save()
                    user.shopping_wallet = 0
                    # user.save()
                elif plan == '70' and directs > idv.extra:
                    ...
                    while x <= count:
                        if user.shopping_wallet > 500:
                            ...
                            x += 1
                self.stdout.write('{} job/s completed with directs {} and user {}'.format(level, directs, user))
        ...
        # for user in neftusers:
            ...
            try:
                ...
                verify = False
            if withuser.new_funds >= 500 and verify==True:
